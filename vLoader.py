...
class Dataset(data.Dataset):
    def __init__(self,contentPath,stylePath,fineSize,device):
        ...
        self.se5=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_5_e.pt'),map_location=device).float()
        self.sv5=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_5_v.pt'),map_location=device).float()
        self.se4=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_4_e.pt'),map_location=device).float()
        self.sv4=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_4_v.pt'),map_location=device).float()
        self.se3=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_3_e.pt'),map_location=device).float()
        self.sv3=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_3_v.pt'),map_location=device).float()
        self.se2=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_2_e.pt'),map_location=device).float()
        self.sv2=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_2_v.pt'),map_location=device).float()
        self.se1=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_1_e.pt'),map_location=device).float()
        self.sv1=torch.load(os.path.join('styles_svd',stylePath.split('/')[-1].split('.')[0]+'_1_v.pt'),map_location=device).float()
